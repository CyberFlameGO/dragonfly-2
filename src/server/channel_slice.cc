...
auto ChannelSlice::FetchSubscribers(string_view channel) -> vector<Subscriber> {
  ...
  if (it != channels_.end()) {
    ...
    CopySubsribers(it->second->subscribers, &res);
  }
  ...
  for (const auto& k_v : patterns_) {
    ...
    if (stringmatchlen(pat.data(), pat.size(), channel.data(), channel.size(), 0) == 1) {
      CopySubsribers(k_v.second->subscribers, &res);
    }
  }
...
void ChannelSlice::CopySubsribers(const SubsribeMap& src, vector<Subscriber>* dest) {
